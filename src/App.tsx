...
const heroHome = "https://www.figma.com/api/mcp/asset/2a1c66bd-bd64-4a9a-9425-2e48431e6c6b";
...
function Header({ page, go, open, lang, setLang }: { page: Page; go: (p: Page) => void; open: (m: ModalType) => void; lang: string; setLang: (v: string) => void }) {
  ...
  const navigate = (next: Page) => { go(next); setMenuOpen(false); };
  const openModal = (modal: ModalType) => { open(modal); setMenuOpen(false); };
  return (
    <header className="topbar">
      <button className="brand" onClick={() => navigate("home")}><Logo /></button>
      <nav className="desktop-nav">
        <button className={page === "home" ? "active" : ""} onClick={() => navigate("home")}>Início</button>
        <button className={page === "map" ? "active" : ""} onClick={() => navigate("map")}>Mapa</button>
        <button onClick={() => openModal("help")}>Sobre</button>
        <button onClick={() => openModal("help")}>Tutorial</button>
        <button onClick={() => openModal("help")}>Contato</button>
      </nav>
      <div className="top-actions">
        ...
        <Button variant="secondary" size="sm" onClick={() => navigate("login")}><LogIn size={16}/>Entrar</Button>
        <Button variant="outline" size="sm" onClick={() => openModal("register")}><User size={16}/>Cadastrar-se</Button>
      </div>
      <button className="mobile-menu-button" type="button" aria-expanded={menuOpen} aria-label="Abrir menu" onClick={() => setMenuOpen((current) => !current)}>{menuOpen ? <X size={24}/> : <Menu size={24}/>}</button>
      <div className={cn("mobile-menu-panel", menuOpen && "open")}>
        <button className={page === "home" ? "active" : ""} onClick={() => navigate("home")}>Início</button>
        <button className={page === "map" ? "active" : ""} onClick={() => navigate("map")}>Mapa</button>
        <button onClick={() => openModal("help")}>Sobre</button>
        <button onClick={() => openModal("help")}>Tutorial</button>
        ...
        <div className="mobile-menu-actions">
          <Button variant="primary" size="sm" onClick={() => navigate("login")}><LogIn size={16}/>Entrar</Button>
          <Button variant="outline" size="sm" onClick={() => openModal("register")}><User size={16}/>Cadastrar-se</Button>
        </div>
      </div>
    </header>
